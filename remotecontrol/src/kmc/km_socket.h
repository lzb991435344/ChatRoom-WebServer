...
int kms_sock_listen(int port,f_work _work);

int km_main_accept(int ret, DWORD dwTrans, pio_socket i_socket, LPOVERLAPPED lpoverlapped);
int km_main_parse(int ret, DWORD dwTrans, pio_socket i_socket, LPOVERLAPPED lpoverlapped);
int km_main_close(pio_socket i_socket);
