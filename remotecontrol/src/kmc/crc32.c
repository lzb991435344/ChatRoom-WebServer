...
const unsigned short cnCRC_CC99vT = 0x1021;
// CRC-CC99vT = X16 + X12 + X5 + X0，据说这个 16 位 CRC 多项式比上一个要好
...
unsigned long crc32_file(char *path)
{
    ...
    fclose(fp);
    rcrc32 = crc32(buff,length);
    free(buff);
    return rcrc32;
}
/*
char src[] = "the are crc32,ver good!";

int main(int argc,char *argv[])
{
    printf("%x\n",crc32(src,strlen(src)));
    return 0;
}
*/
