...
http_response* http_response_init() {
    http_response *resp;//回应结构体的指针
    resp = malloc(sizeof(*resp));//分配内存空间
    memset(resp, 0, sizeof(*resp));//初始化

    resp->headers = http_headers_init();//头部
    resp->entity_body = string_init();//实体
    resp->content_length = -1;//内容长度

    return resp;//返回结构体的指针
}
